...
class Post extends Model
{
    ...
    public function getHrefAttribute()
    {
        ...
        if (!empty($this->attributes['slug'])) {
            $href = url("blog/{$this->attributes['slug']}");
        }
    ...
    public function getExcerptAttribute()
    {
        ...
        $content_markdown = $this->findFirstMatchingKeyValue($content, 'markdown');
        if ($content_markdown) {
            $excerpt = Str::words(strip_tags($content_markdown), 50);
        }
    ...
    {
        return $this->belongsToMany(PostCategory::class)->withTimestamps();
    }

    public function relatedByCategories($limit = 5)
    {
        $category_ids = $this->categories->pluck('id')->toArray();

        return static::where('id', '!=', $this->id)->whereHas('categories', function ($query) use ($category_ids) {
