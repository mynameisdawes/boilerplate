...
class Countries
{
    ...
    public static function populate()
    {
        ...
        if (
            ...
            foreach (self::$countries_src as $_country) {
                if ($_country[$allowed_countries_property] == $user->configuration['shipping_country']) {
                    $allowed_countries = [
                        $_country[$allowed_countries_property],
                    ];
        ...
        $countries = array_filter(self::$countries_src, function ($_country) use ($allowed_countries, $allowed_countries_property) {
            return (null != $allowed_countries) ? in_array($_country[$allowed_countries_property], $allowed_countries) : true;
        });
    ...
    public static function convert($value, $source = 'name', $destination = 'iso2')
    {
        ...
        $countries = array_filter(self::$countries_src, function ($country) use ($value, $source) {
